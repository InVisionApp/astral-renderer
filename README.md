...
is untested and unsupported at this time.
...
Intel integrated GPU, one can recover performance by using OpenGL ES
using ANGLE. Tests on Linux have shown that the issue is NOT the GPU or
Astral; it is the GL drivers from Apple for these GPU's that is the issue.
...
search path which is controlled by `DYLD_LIBRARY_PATH`.
...
application build and to aslo add the correct Astral library; to use the debug
