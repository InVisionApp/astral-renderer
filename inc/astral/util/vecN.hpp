...
#ifndef ASTRAL_VECN_HPP
#define ASTRAL_VECN_HPP
...
public:
   ...
   */
  vecN&
  operator=(const vecN &rhs) = default;

  /*!
   * Move ctor
   * \param obj value from which to move values
   */
  vecN(vecN &&obj) = default;

  /*!
   * assignment operator
   * \param rhs value from which to assign-move values
   */
  vecN&
  operator=(vecN &&rhs) = default;
   ...
   * \param default_value value to give elements beyond M
   */
  template<size_t M>
   ...
   */
  reference
  x(void) { ASTRALstatic_assert(N >= 1); return c_ptr()[0]; }
   ...
   */
  reference
  y(void) { ASTRALstatic_assert(N >= 2); return c_ptr()[1]; }
   ...
   */
  reference
  z(void) { ASTRALstatic_assert(N >= 3); return c_ptr()[2]; }
   ...
   */
  reference
  w(void) { ASTRALstatic_assert(N >= 4); return c_ptr()[3]; }
   ...
   */
  const_reference
  x(void) const { ASTRALstatic_assert(N >= 1); return c_ptr()[0]; }
   ...
   */
  const_reference
  y(void) const { ASTRALstatic_assert(N >= 2); return c_ptr()[1]; }
   ...
   */
  const_reference
  z(void) const { ASTRALstatic_assert(N >= 3); return c_ptr()[2]; }
   ...
   */
  const_reference
  w(void) const { ASTRALstatic_assert(N >= 4); return c_ptr()[3]; }
   ...
   * \param abs_tol absolute tolerance of comparison.
   */
  bool
  approximately_equal(const vecN &obj, T rel_tol, T abs_tol) const
  {
    T thresh, m;

    m = astral::t_max(L1norm(), obj.L1norm());
    thresh = astral::t_max(abs_tol, m * rel_tol);
    for (unsigned int i = 0; i < N; ++i)
      {
        if (astral::t_abs(operator[](i) - obj[i]) > thresh)
          {
            return false;
          }
      }
  ...
  /*!
   * Conveniant overload, equivalent to
   * \code
   * approximately_equal(obj, rel_tol, std::numeric_limits<T>::epsilon());
   * \endcode
   */
  bool
  approximately_equal(const vecN &obj, T rel_tol) const
  {
    return approximately_equal(obj, rel_tol, std::numeric_limits<T>::epsilon());
  }
...
/*!
 * Returns the component-wise max of two vecN objects
 */
template<typename T, size_t N>
vecN<T, N>
component_max(const vecN<T, N> &a,
              const vecN<T, N> &b)
{
  vecN<T, N> R;
...
/*!
 * Returns the component-wise min of two vecN objects
 */
template<typename T, size_t N>
vecN<T, N>
component_min(const vecN<T, N> &a,
              const vecN<T, N> &b)
{
  vecN<T, N> R;
 ...
 * Conveniance typedef
 */
typedef vecN<generic_data, 1> gvec1;
/*!
 * Conveniance typedef
 */
typedef vecN<generic_data, 2> gvec2;
/*!
 * Conveniance typedef
 */
typedef vecN<generic_data, 3> gvec3;
/*!
 * Conveniance typedef
 */
typedef vecN<generic_data, 4> gvec4;
 ...
 * Conveniance typedef
 */
typedef vecN<bool, 1> bvec1;
/*!
 * Conveniance typedef
 */
typedef vecN<bool, 2> bvec2;
/*!
 * Conveniance typedef
 */
typedef vecN<bool, 3> bvec3;
/*!
 * Conveniance typedef
 */
typedef vecN<bool, 4> bvec4;
...
/*!
 * Pack an astral::ivec2 where each of its
 * coordinate values is non-negative and
 * no more than 0x7FFF into a single uint32_t
 */
inline
uint32_t
pack_pair(ivec2 v)
{
  ASTRALassert(v.x() >= 0);
  ASTRALassert(v.y() >= 0);
  return pack_pair(static_cast<unsigned int>(v.x()), static_cast<unsigned int>(v.y()));
}

/*!
 * Unpack an astral::ivec2 where each of its
 * coordinate values is non-negative and
 * no more than 0xFFF from a single uint32_t
 */
inline
ivec2
unpack_pair(uint32_t v)
{
  uvec2 r;
  unpack_pair(v, &r.x(), &r.y());
  return ivec2(static_cast<int>(r.x()), static_cast<int>(r.y()));
}

/*!
 * Pack a pair of uint32_t values each of which
 * is no more than 0xFFFF into a single uint32_t.
 * \param v uvec2 where each channel has value no
 *          more than 0xFFFu
 */
inline
uint32_t
pack_pair(uvec2 v)
{
  return pack_pair(v.x(), v.y());
}

/*!
 * Pack an astral::u8vec4 value into a single
 * uint32_t value.
 */
inline
uint32_t
pack_u8vec4(u8vec4 v)
{
  return pack_bits(0u, 8u, v.x())
    | pack_bits(8u, 8u, v.y())
    | pack_bits(16u, 8u, v.z())
    | pack_bits(24u, 8u, v.w());
}
...
/*!
 * Compute the linear interpolation of two vectors
 */
template<typename T, size_t N>
vecN<T, N>
mix(const vecN<T, N> &p0, const vecN<T, N> &p1, T t)
{
  T s(T(1) - t);
  return s * p0 + t * p1;
}
...
/*!
 * Compute the linear interpolation of two scalars
 */
template<typename T>
T
mix(const T &p0, const T &p1, T t)
{
  T s(T(1) - t);
  return s * p0 + t * p1;
}
